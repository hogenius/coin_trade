...
def check_sell_hmm(coin_info, balances, config, simple_data:SimpleData, print_msg, isForce, isTest):
    ...
    is_update = False
    if latest_sell_check_timestamp == None or (latest_sell_check_timestamp != None and latest_sell_check_timestamp < latest_timestamp):
        is_update = True
    latest_sell_check_timestamp = latest_timestamp
    ...
    arr_send_msg = []
    # 매도 신호 조건
    sell_signal = current_state == bullish_state and (next_state_probs[stable_state] > 0.2 or next_state_probs[bearish_state] > 0.2)
